...
function countStringInLine( lineValue, valuesToFind ) {
  let countOfTags = 0;
  ...
  R.forEach( ( valueToFind ) => {
    ...
    if ( matchedValues ) {
      if ( R.is( Array, matchedValues ) ) {
        countOfTags += matchedValues.length;
      } else {
        countOfTags += 1;
      }
    }
...
function countStringInCommitMessage( validatorName, validatorProperty, messageArray ) {
  ...
  R.forEach( ( lineValue ) => {
    count += countStringInLine( lineValue, valuesToFind );
  }, messageArray );
