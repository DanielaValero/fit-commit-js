...
function isOnlyOneAllowed() {
  return base.getValidatorProperty(
    ...
    constants.ONLY_ONE_TICKET_PROPERTY,
  )
                                    || constants.ONLY_ONE_TICKET_DEFAULT_VALUE;
}
...
function validateticketCode( messageArray ) {
  const count = utils.countStringInCommitMessage(
    constants.VALIDATOR_NAME,
    constants.TICKET_CODE_PROPERTY
                                                  || constants.TICKET_CODE_DEFAULT_VALUE,
    messageArray,
  );
