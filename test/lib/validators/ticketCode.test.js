...
suite( 'Validators.ticketCode | ', () => {
  ...
  const fixturesPathOnlyOneTicket = path.resolve( __dirname, '../../fixtures/validators/ticketCode' );
  ...
  test( 'More than one ticket code per commit are allowed', () => {
    const validLines = [ 'feature: first line', 'FITCOMMITJS-123, FITCOMMITJS-124', `Fourth line`, 'fifth line' ];
    const hasErrors = ticketCode.validate( validLines );
  ...
  test( 'Only one ticket per commit is allowed', () => {
    validatorsConfig.clearEnabledValidators();
    validatorsConfig.loadEnabledValidatorsObject( fixturesPathOnlyOneTicket );
    const validLines = [ 'feature: first line', 'FITCOMMITJS-123, FITCOMMITJS-124', `Fourth line`, 'fifth line' ];
    const hasErrors = ticketCode.validate( validLines );
